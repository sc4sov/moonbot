...
function update()
    local raw = 'https://raw.githubusercontent.com/sc4sov/moonbot/main/update.json'
        ...
        if response.status_code == 200 then
            downloadUrlToFile(decodeJson(response.text)['url'], thisScript().path, function (id, status, p1, p2)
                if status == dlstatus.STATUSEX_ENDDOWNLOAD then
                    chat_message("Обновление скрипта завершено")
                    if not script.find('ML%-AutoReboot') then thisScript():reload() end
                end
            end)
        else
			chat_message(string.format("Ошибка при обновлении скрипта. Код: {ffdead}#%d", response.status_code))
        end
    end
    return f
end
...
function main() 
    ...
    if tonumber(thisScript().version) < tonumber(lastver) then
		chat_message(string.format("Сейчас произойдет обновление скрипта до версии {ffdead}%s (Текущая версия: %s)", lastver, thisScript().version))
        update():download()
        return
    end
    chat_message('Скрипт готов к работе. Команда: {FFDEAD}/a.info{ffffff}')
    if tonumber(thisScript().version) > tonumber(lastver) then
        chat_message(string.format("У вас установлена developer версия: {ffdead}%s",  thisScript().version))
    end
